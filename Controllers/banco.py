...
def consulta_fecoep():
        ...
        consulta = """SELECT T2.CD_MATERIAL AS COD, T2.DS_MATERIAL AS DESCRICAO, T1.CD_UF AS UF ,T1.PR_FCP_MATERIAL AS FECOEP, T2.CD_GRADE_TRIBUTARIA AS CD_GRADE, T3.DS_GRADE AS GRADE, T2.CD_FILIAL_MATERIAL AS FILIAL, (CASE WHEN T2.X_CALCULA_FCP = 1 THEN 'SIM' ELSE 'NÃO' END) AS CALC_FCP, (CASE WHEN T2.X_CALCULA_FCP_ST = 1 THEN 'SIM' ELSE 'NÃO' END) AS CALC_FCP_ST
						...
						UNION 
						SELECT T2.CD_MATERIAL AS COD, T2.DS_MATERIAL AS DESCRICAO, T1.CD_UF AS UF ,T1.PR_FCP_MATERIAL AS FECOEP, T2.CD_GRADE_TRIBUTARIA AS CD_GRADE, T3.DS_GRADE AS GRADE, T2.CD_FILIAL_MATERIAL AS FILIAL, (CASE WHEN T2.X_CALCULA_FCP = 1 THEN 'SIM' ELSE 'NÃO' END) AS CALC_FCP, (CASE WHEN T2.X_CALCULA_FCP_ST = 1 THEN 'SIM' ELSE 'NÃO' END) AS CALC_FCP_ST
                        FROM TBL_MATERIAIS_ALIQUOTA_UF_DIFAL_FCP T1
                        RIGHT JOIN TBL_MATERIAIS T2 ON T1.CD_MATERIAL = T2.CD_MATERIAL
                        INNER JOIN TBL_GRADE_TRIBUTARIA T3 ON T2.CD_GRADE_TRIBUTARIA = T3.CD_GRADE
                        WHERE T2.CD_GRADE_TRIBUTARIA NOT IN (33,37,38)
